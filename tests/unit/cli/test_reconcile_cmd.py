...
def test_reconcile_command_dry_run(mock_deps):
    ...
    with (
        patch(
            "zotero_cli.core.services.slr.orchestrator.SLROrchestrator.get_all_papers_in_tree"
        ) as gap,
...
def test_reconcile_command_execute(mock_deps):
    ...
    with (
        ...
        args = argparse.Namespace(
            tree="raw_test", execute=True, verbose=False, user=False, qa_threshold=2.0
        )
        ReconcileCommand.execute(args)
