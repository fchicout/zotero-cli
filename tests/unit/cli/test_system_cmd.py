...
def test_system_normalize(system_cmd, capsys, tmp_path):
    ...
    with (
        patch("zotero_cli.core.strategies.CanonicalCsvImportStrategy.fetch_papers") as mock_fetch,
        patch("zotero_cli.infra.canonical_csv_lib.CanonicalCsvLibGateway.write_file"),
    ):
...
def test_system_groups(system_cmd, capsys):
    with (
        ...
        mock_gw = mock_gw_factory.return_value
        mock_gw.get_user_groups.return_value = [{"id": 123, "data": {"name": "Lab A"}}]
...
def test_system_jobs_run(system_cmd, capsys):
    with (
        ...
        args = argparse.Namespace(
            verb="jobs", jobs_verb="run", type="fetch_pdf", count=10, watch=False
        )
        system_cmd.execute(args)
