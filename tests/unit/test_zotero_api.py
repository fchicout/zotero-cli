...
def test_get_items_by_tag_pagination(client):
    ...
    res1.json.return_value = [
        {"key": f"K{i}", "data": {"title": f"T{i}"}, "meta": {}, "version": 1} for i in range(100)
    ]
...
def test_get_items_in_collection_top_only(client):
    ...
    mock_response.status_code = 200
    mock_response.json.return_value = [
        {"key": "K1", "data": {"title": "Top Item"}, "meta": {}, "version": 1}
    ]
    mock_response.headers = {}
    client.http.session.get.return_value = mock_response
...
def test_upload_attachment_full_sequence(
    mock_req_post, mock_file, mock_mtime, mock_basename, mock_getsize, client
):
