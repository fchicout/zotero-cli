...
def test_lookup_items_table(lookup_service, mock_gateway):
    ...
    output = lookup_service.lookup_items(
        keys, fields=["key", "title", "arxiv_id"], output_format="table"
    )
...
def test_lookup_items_clean_fields(lookup_service, mock_gateway):
    keys = ["DIRTY"]
    item1 = ZoteroItem(
        key="DIRTY", version=1, item_type="journalArticle", title="Paper | With | Pipes"
    )
    mock_gateway.get_item.return_value = item1
