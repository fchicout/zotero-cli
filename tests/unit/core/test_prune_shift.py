...
def test_prune_intersection(col_service, mock_col_repo, mock_item_repo):
    ...
    item_a_inc = ZoteroItem.from_raw_zotero_item(
        {
            "key": "A",
            "data": {
                "title": "Paper A",
    ...
    item_a_exc = ZoteroItem.from_raw_zotero_item(
        {
            "key": "A",
            "data": {
                "title": "Paper A",
    ...
    mock_col_repo.get_items_in_collection.side_effect = (
        lambda x, **kwargs: iter([item_a_inc]) if x == "1" else iter([item_a_exc])
    )
