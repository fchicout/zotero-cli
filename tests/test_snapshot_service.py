...
def test_freeze_collection_success(snapshot_service, mock_gateway, tmp_path):
    ...
    # Mock Items
    item1 = ZoteroItem(
        key="ITEM1", version=1, item_type="journalArticle",
        title="Paper 1", abstract="Abstract 1", authors=["Author A"]
    )
    item2 = ZoteroItem(
        key="ITEM2", version=1, item_type="journalArticle",
        title="Paper 2", abstract="Abstract 2", authors=["Author B"]
    )
    mock_gateway.get_items_in_collection.return_value = iter([item1, item2])
    
    # Mock Children
    mock_gateway.get_item_children.side_effect = [
        [{"key": "NOTE1", "itemType": "note", "note": "<div>Decision: INCLUDE</div>"}], # Children for ITEM1
        [] # Children for ITEM2
    ]
