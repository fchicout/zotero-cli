...
class TestCLI(unittest.TestCase):
    ...
    def test_springer_csv_command_success(self, mock_stdout, MockZoteroAPI, MockArxivLibGateway, MockBibtexLibGateway, MockRisLibGateway, MockSpringerCsvLibGateway, MockIeeeCsvLibGateway, MockPaperClient): # Updated args
        # Setup mocks
        mock_client_instance = MockPaperClient.return_value
        mock_client_instance.import_from_springer_csv.return_value = 5

        # Simulate 'springer-csv' subcommand
        test_args = ['program', 'springer-csv', '--file', 'springer.csv', '--folder', 'F']
        with patch.object(sys, 'argv', test_args):
            main()
    ...
    def test_ieee_csv_command_success(self, mock_stdout, MockZoteroAPI, MockArxivLibGateway, MockBibtexLibGateway, MockRisLibGateway, MockSpringerCsvLibGateway, MockIeeeCsvLibGateway, MockPaperClient): # New test
        # Setup mocks
        mock_client_instance = MockPaperClient.return_value
        mock_client_instance.import_from_ieee_csv.return_value = 6

        # Simulate 'ieee-csv' subcommand
        test_args = ['program', 'ieee-csv', '--file', 'ieee.csv', '--folder', 'F']
        with patch.object(sys, 'argv', test_args):
            main()
    ...
    @patch('sys.stdout', new_callable=StringIO)
    def test_add_command_collection_not_found(self, mock_stdout, mock_stderr, MockZoteroAPI, MockArxivLibGateway, MockBibtexLibGateway, MockRisLibGateway, MockSpringerCsvLibGateway, MockIeeeCsvLibGateway, MockPaperClient): # Updated args
        # Setup mocks
        mock_client_instance = MockPaperClient.return_value
