...
def test_collection_backup_e2e(tmp_path):
    ...
    try:
        result = subprocess.run(
            ["zotero-cli", "collection", "backup", "--name", "NonExistentCollection", "--output", str(output_file)],
            capture_output=True,
            text=True,
            timeout=30
        )
...
def test_backup_logic_flow_with_real_zip(tmp_path):
    ...
    mock_gateway.get_items_in_collection.return_value = [
        ZoteroItem.from_raw_zotero_item({"key": "ITEM1", "data": {"title": "Paper", "version": 1, "itemType": "journalArticle"}})
    ]
