...
def run_cli(args):
    ...
    return subprocess.run([sys.executable, "-m", "zotero_cli.cli.main"] + args, capture_output=True, text=True, env=env)
...
def test_shift_e2e(tmp_path):
    ...
    try:
        ...
        run_cli(["import", "arxiv", "--query", "id:1706.03762", "--limit", "1", "--collection", col_a])
        ...
        run_cli(["review", "decide", "--key", key, "--vote", "INCLUDE", "--code", "SHIFT", "--source", col_a, "--target", col_b])
        
        time.sleep(5)
        run_cli(["report", "snapshot", "--collection", col_b, "--output", str(snap2)])
