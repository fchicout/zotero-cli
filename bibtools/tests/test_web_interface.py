...
from bibtools.web.app import app
...
@pytest.fixture
def sample_springer_csv():
    """Create a sample Springer CSV file for article extraction testing."""
    csv_content = """DOI,Title,Abstract Note,Publication Title,Authors,URL,Publication Year,Content Type,Journal Volume,Journal Issue
10.1007/test1,"Machine Learning Applications in Healthcare","This paper explores the application of machine learning algorithms in healthcare diagnostics and treatment planning.","Journal of Medical AI","John Smith; Jane Doe",https://example.com/paper1,2023,Article,10,2
10.1007/test2,"Deep Learning for Image Recognition","A comprehensive study of deep learning techniques for image recognition tasks.","Computer Vision Conference","Alice Johnson",https://example.com/paper2,2024,Conference Paper,15,3
"""
    return csv_content.encode('utf-8')


def test_extract_articles_get_route(client):
    """Test that the extract articles page loads successfully (Requirement 7.1)."""
    response = client.get('/extract-articles')
    assert response.status_code == 200
    # Should contain the upload form
    assert b'extract' in response.data.lower() or b'upload' in response.data.lower()


def test_extract_articles_no_file(client):
    """Test article extraction endpoint with no file uploaded."""
    response = client.post('/extract-articles')
    assert response.status_code == 400
    data = response.get_json()
    assert data['success'] is False
    assert 'No file uploaded' in data['error']
...
def test_extract_articles_empty_filename(client):
    """Test article extraction endpoint with empty filename."""
    data = {'csv_file': (BytesIO(b''), '')}
    response = client.post('/extract-articles', data=data, content_type='multipart/form-data')
    assert response.status_code == 400
    json_data = response.get_json()
    assert json_data['success'] is False
    assert 'No file selected' in json_data['error']
...
def test_extract_articles_invalid_file_type(client):
    """Test article extraction endpoint with non-CSV file (Requirement 7.2)."""
    data = {'csv_file': (BytesIO(b'test content'), 'test.txt')}
    response = client.post('/extract-articles', data=data, content_type='multipart/form-data')
    assert response.status_code == 400
    json_data = response.get_json()
...
def test_extract_articles_valid_csv(client, sample_springer_csv):
    """Test successful article extraction from a valid CSV file (Requirements 7.2, 7.3)."""
    data = {'csv_file': (BytesIO(sample_springer_csv), 'test_springer.csv')}
    response = client.post('/extract-articles', data=data, content_type='multipart/form-data')
    ...
    json_data = response.get_json()
    assert json_data['success'] is True
    assert json_data['records_count'] == 2
    assert 'filename' in json_data
    assert 'download_url' in json_data
    assert json_data['filename'].endswith('.xlsx')


def test_extract_articles_download(client, sample_springer_csv):
    """Test successful download of extracted Excel file (Requirement 7.5)."""
    # First, extract articles
    data = {'csv_file': (BytesIO(sample_springer_csv), 'test_springer.csv')}
    response = client.post('/extract-articles', data=data, content_type='multipart/form-data')
    assert response.status_code == 200
    ...
    # Download the file
    download_response = client.get(f'/download-articles/{filename}')
    
    # Verify successful download
    assert download_response.status_code == 200
    assert len(download_response.data) > 0
    
    # Verify correct content-type header for Excel files (Requirement 7.5)
    assert 'spreadsheet' in download_response.headers.get('Content-Type', '').lower()
    
    # Verify content-disposition header for download
    assert download_response.headers.get('Content-Disposition') is not None


def test_extract_articles_empty_csv(client):
    """Test handling of empty CSV file (Requirement 1.4)."""
    empty_content = b'DOI,Title,Abstract Note\n'
    data = {'csv_file': (BytesIO(empty_content), 'empty.csv')}
    response = client.post('/extract-articles', data=data, content_type='multipart/form-data')
    # Should fail with appropriate error
    json_data = response.get_json()
    assert json_data['success'] is False
    # The error can come from either CSV validation or the extractor
    assert 'error' in json_data


def test_extract_articles_malformed_csv(client):
    """Test handling of malformed CSV file (Requirement 1.5)."""
    # CSV with missing required columns
    malformed_content = b'Column1,Column2\nValue1,Value2\n'
    data = {'csv_file': (BytesIO(malformed_content), 'malformed.csv')}
    response = client.post('/extract-articles', data=data, content_type='multipart/form-data')
    ...
    assert json_data['error_type'] == 'invalid_csv'


def test_extract_articles_with_special_characters(client):
    """Test handling of CSV with special characters (Requirement 2.5)."""
    csv_content = b"""DOI,Title,Abstract Note,Publication Title,Authors,URL,Publication Year,Content Type,Journal Volume,Journal Issue
10.1007/test,"Test & Title with <special> chars","Abstract with special chars: \xc3\xa9\xc3\xa0\xc3\xbc","Journal","Author's Name",https://example.com,2023,Article,10,2
"""
    data = {'csv_file': (BytesIO(csv_content), 'special.csv')}
    response = client.post('/extract-articles', data=data, content_type='multipart/form-data')
    # Should handle special characters gracefully
...
def test_extract_articles_error_messages(client):
    """Test that error messages are clear and helpful (Requirement 7.4)."""
    # Test various error scenarios and verify error messages
...
def test_extract_articles_cleanup(client, sample_springer_csv):
    """Test that the cleanup endpoint removes Excel files after download."""
    # First, extract articles
    data = {'csv_file': (BytesIO(sample_springer_csv), 'test_springer.csv')}
    response = client.post('/extract-articles', data=data, content_type='multipart/form-data')
    assert response.status_code == 200
    ...
    # Call cleanup endpoint
    cleanup_response = client.post(f'/cleanup-articles/{filename}')
    assert cleanup_response.status_code == 200
...
def test_extract_articles_path_traversal_prevention(client):
    """Test that path traversal attempts are blocked (Requirement 7.2)."""
    # Try to access a file outside the output directory
    response = client.get('/download-articles/../../../etc/passwd')
    # Should either return 404 or 400 (security error)
    assert response.status_code in [404, 400]
    
    # Try with encoded path traversal
    response = client.get('/download-articles/..%2F..%2Fetc%2Fpasswd')
    assert response.status_code in [404, 400]


def test_extract_articles_file_size_limit(client):
    """Test that files exceeding size limit are rejected (Requirement 7.2)."""
    ...
    json_data = response.get_json()
    assert json_data['success'] is False
    assert 'size' in json_data['error'].lower()


def test_extract_articles_concurrent_uploads(client, sample_springer_csv):
    """Test handling of concurrent file uploads."""
    # Simulate multiple uploads
    responses = []
    for i in range(3):
        data = {'csv_file': (BytesIO(sample_springer_csv), f'test{i}.csv')}
        response = client.post('/extract-articles', data=data, content_type='multipart/form-data')
        responses.append(response)
...
def test_extract_articles_missing_fields(client):
    """Test handling of records with missing fields (Requirement 1.3)."""
    csv_content = b"""DOI,Title,Abstract Note,Publication Title,Authors,URL,Publication Year,Content Type,Journal Volume,Journal Issue
10.1007/test1,"Complete Record","Abstract","Journal","Author",https://example.com,2023,Article,10,2
,"Missing DOI","Abstract","Journal","Author",https://example.com,2023,Article,10,2
10.1007/test3,,"Abstract","Journal","Author",https://example.com,2023,Article,10,2
10.1007/test4,"Missing Abstract",,"Journal","Author",https://example.com,2023,Article,10,2
"""
    data = {'csv_file': (BytesIO(csv_content), 'missing_fields.csv')}
    response = client.post('/extract-articles', data=data, content_type='multipart/form-data')
    
    # Should succeed and process all records
    assert response.status_code == 200
    json_data = response.get_json()
    assert json_data['success'] is True
    assert json_data['records_count'] == 4  # All 4 records should be processed
