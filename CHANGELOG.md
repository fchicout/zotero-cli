...
- **`DuplicateFinder` as a Clean Library API (Issue #153, partial):** `DuplicateFinder.find_duplicates`/`compare_collections` now return typed `DuplicateGroup`/`DuplicateOccurrence` dataclasses instead of ad hoc dicts, and no longer `print()` internally — non-fatal issues (e.g. a named collection that doesn't exist) are collected in `self.warnings` for the caller to surface instead. This is the first of four services #153 covers; `MergeService`, the merge-plan dataclasses, and the SLR dedupe service (Issues #155/#156/#157) don't exist yet and will be held to the same standard when built, so #153 stays open until then.
