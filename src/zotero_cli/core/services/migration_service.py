...
class MigrationService:
                ...
                if child_data.get("itemType") == "note":
                    ...
                    if self._is_screening_note(note_content):
                        ...
                        if new_content != note_content:
                            if not dry_run:
                                success = self.gateway.update_note(
                                    note_key, note_version, new_content
                                )
                                if success:
                                    stats["migrated"] += 1
                                else:
    ...
    def _is_screening_note(self, content: str) -> bool:
        return (
            "screening_decision" in content
            or "audit_version" in content
    ...
    def _migrate_note_content(self, content: str) -> str:
        ...
        try:
            ...
            if data.get("agent") != "zotero-cli":
                ...
                if data.get("agent") and data.get("agent") not in [
                    "zotero-cli",
                    "zotero-cli-tui",
                    "zotero-cli-agent",
                ]:
