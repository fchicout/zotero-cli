...
class ReportService:
    ...
    def generate_pdf_report(self, jobs: List[Job]) -> str:
        """Generates a Markdown report of the PDF discovery results."""
        md = [
            "# PDF Discovery Report",
            f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            "",
            "| Item Key | Status | Method | Attempts | Result/Error |",
            "| :--- | :--- | :--- | :---: | :--- |",
        ]
        ...
        md.extend(
            [
                ...
                f"**Success:** {len([j for j in jobs if j.status == 'COMPLETED'])}",
                f"**Failed:** {len([j for j in jobs if j.status == 'FAILED'])}",
                "",
                "---",
                "*Generated by zotero-cli PDF Resilience Engine.*",
            ]
        )
